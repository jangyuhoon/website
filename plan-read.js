...
function getPost(id) { // Removed category parameter
    ...
    return posts.find(post => post.id === id && post.category === 'plan'); // Hardcode 'plan'
}
...
function loadPost() {
    ...
    if (!postId) { // Removed !currentCategory check
        ...
        window.location.href = 'plan.html'; // Hardcode 'plan.html'
        return;
    }
...
function updatePost(updatedPost) {
    const posts = getPosts();
    const index = posts.findIndex(post => post.id === updatedPost.id && post.category === 'plan'); // Hardcode 'plan'
